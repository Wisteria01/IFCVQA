...
class DataSet(Data.Dataset):
    def __init__(self, __C):
        ...
        #load cpv2
        self.stat_ques_list = (
            json.load(open(__C.QUESTION_PATH['train'], 'r')) +  # 直接加载训练集
            json.load(open(__C.QUESTION_PATH['val'], 'r'))['questions'] +     # 直接加载验证集
            json.load(open(__C.QUESTION_PATH['test'], 'r')) +    # 直接加载测试集
            json.load(open(__C.QUESTION_PATH['vg'], 'r'))['questions']        # 直接加载VG数据集
        )
        """
